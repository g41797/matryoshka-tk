...
**Version 003.** It supersedes `3tk-staging-plan-002.md`, which supersedes
`3tk-staging-plan-001.md`. Both stay on disk. The only change in 003 is the
addition of 3TK-7, on the owner's instruction of 2026-08-23. Stages 3TK-0 to
3TK-6 are reproduced unaltered — they are done, and a plan version does not
rewrite history.
...
- this plan — `3tk-staging-plan-NNN.md`, currently 003
- `ztk-audit-NNN.md`
...
- 3TK-6 and 3TK-7 write C3 under `c3/3tk/`. Still inside `c3/`, so the storage
  rule holds and no kitchen gate applies. `src/` at the repository root is the
