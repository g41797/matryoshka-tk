# 3tk — API verification table, 001
...
- **Contract** — `@param to : "the handle this item now links to; itself, if it is the end"` — `inner.c3:121`.
...
- **Costs** — O(1).
- **Contract** — `@param h : "the handle to place; must not be null"` — `inner.c3:324`.
- **Checks** — `@check(h != null, "Slot.fill with a null handle")` — `inner.c3:328`.
- **Checks** — `@check(self.is_empty(), "Part 9.2 rule 1: never overwrite a full Slot")` — `inner.c3:329`.
...
- **Contract** — `@param #cond : "the condition that must hold"` — `inner.c3:213`.
- **Contract** — `@param $msg : "a compile-time message, named at the call site"` — `inner.c3:214`.
...
- **Contract** — `@param $Type : "the outer type"` — `inner.c3:347`.
- **Checks** — `$assert $seen != 0 : "type " +++ $Type::name +++ " has no Inner field; it cannot be a Matryoshka item"` — `inner.c3:359`.
- **Checks** — `$assert $seen == 1 : "type " +++ $Type::name +++ " has more than one Inner field; Part 4.4 allows exactly one"` — `inner.c3:360`.
...
- **Costs** — compile time only.
- **Contract** — `@param $Type : "the outer type"` — `inner.c3:375`.
- **Checks** — `$assert $off >= 0 : "type " +++ $Type::name +++ " has no Allocator field; use mtk::helper instead of mtk::managed"` — `inner.c3:385`.

**A second declaration of the same name lives in `managed.c3:117`**, with the
same body and the same `$assert` at `managed.c3:125`.
...
- **Checks** — `@check(h != null, "insert of a null handle")` — `queue.c3:90`.
- **Checks** — `@check(!is_linked(h), "Part 8.7: the item is already on a chain")` — `queue.c3:91`.
...
- **Checks** — `@check(s.is_full(), "push_back_slot from an empty Slot")` — `queue.c3:167`.
...
- **Contract** — `@param other : "the queue to empty onto this one; empty afterwards"` — `queue.c3:215`.
- **Checks** — `@check(other != null, "append_queue with a null queue")` — `queue.c3:219`.
- **Checks** — `@check(other != self, "Part 8.9: a queue cannot be moved onto itself")` — `queue.c3:220`.
...
- **Checks** — `@check(h != null, "insert of a null handle")` — `stack.c3:81`.
- **Checks** — `@check(!is_linked(h), "Part 8.7: the item is already on a chain")` — `stack.c3:82`.
...
- **Contract** — `@param $Type : "the outer type"` — `helper.c3:74`.
...
- **Costs** — O(1), one `any_make`. The type is inferred from the pointer; no
  call site names it.
- **Contract** — `@param item : "a pointer to the outer"` — `helper.c3:97`.
- **Contract** — `@require $defined($Typeof(*item)::members) : "not a struct that embeds an Inner"` — `helper.c3:98`.
...
- **Contract** — `@param item : "a pointer to the outer, or null"` — `helper.c3:112`.
- **Contract** — `@require $defined($Typeof(*item)::members) : "not a struct that embeds an Inner"` — `helper.c3:113`.
...
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:128`.
...
- **Contract** — `@param $Type : "the outer type asserted"` — `helper.c3:152`.
- **Contract** — `@require is_mine(h, $Type) : "the handle is not of this type"` — `helper.c3:153`.
...
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:162`.
...
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type asserted"` — `helper.c3:169`.
...
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:187`.
...
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:205`.

### `macro Inner.as(&self, $Type)`

- **What** — the asserting crossing, as a method.
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type asserted"` — `helper.c3:212`.
- **Contract** — `@require is_mine((Handle)self, $Type) : "the handle is not of this type"` — `helper.c3:213`.

### `macro Slot.to(&self, $Type)`

- **What** — the checking Slot crossing, as a method. Forwards to `from_slot`.
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:220`.

### `macro Slot.must(&self, $Type)`

- **What** — the asserting Slot crossing, as a method.
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type asserted"` — `helper.c3:225`.

### `macro Slot.move(&self, $Type)`

- **What** — the moving Slot crossing, as a method.
- **Costs** — O(1).
- **Contract** — `@param $Type : "the outer type expected"` — `helper.c3:230`.
...
- **Costs** — one allocation, through `alloc::new_try`. The plain `alloc::new`
  ...
  true on.
- **Contract** — `@param $Type : "the outer type, which must carry an Allocator field"` — `managed.c3:71`.
- **Contract** — `@param a : "the allocator the item keeps for life"` — `managed.c3:72`.
- **Contract** — `@param slot : "an empty Slot, filled on success"` — `managed.c3:73`.
- **Checks** — `mtk::@check(slot.is_empty(), "Part 9.2 rule 3: an acquisition asserts the Slot is empty on entry")` — `managed.c3:77`.
...
- **Promises** — **no allocator parameter.** A no-op on an empty Slot, which is
  ...
  empty afterwards.
- **Costs** — one free.
- **Contract** — `@param $Type : "the outer type, which must carry an Allocator field"` — `managed.c3:92`.
- **Contract** — `@param slot : "the Slot holding the item; empty afterwards"` — `managed.c3:93`.
...
- **Promises** — the same declaration as `mtk::required_alloc_offset`, with the
  same message.
- **Costs** — compile time only.
- **Contract** — `@param $Type : "the outer type"` — `managed.c3:115`.
- **Checks** — `$assert $off >= 0 : "type " +++ $Type::name +++ " has no Allocator field; use mtk::helper instead of mtk::managed"` — `managed.c3:125`.
...
- **Contract** — `@param a : "the allocator the mailbox keeps for life"` — `mailbox.c3:115`.
...
- **Promises** — **the mailbox must be closed first, and this is the one
  ...
  parameter.
- **Costs** — one free.
- **Checks** — `always_assert(self._closed, "Part 11.12: releasing an open mailbox")` — `mailbox.c3:146`.
...
- **Contract** — `@param slot : "a full Slot; cleared on success, untouched on CLOSED"` — `mailbox.c3:203`.
- **Contract** — `@return? mtk::CLOSED` — `mailbox.c3:204`.
- **Checks** — `mtk::@check(slot.is_full(), "Mailbox.send from an empty Slot")` — `mailbox.c3:223`.
...
- **Contract** — `@param slot : "a full Slot; cleared on success, untouched on CLOSED"` — `mailbox.c3:216`.
- **Contract** — `@return? mtk::CLOSED` — `mailbox.c3:217`.
...
- **Contract** — `@param slot : "an empty Slot; filled on success"` — `mailbox.c3:245`.
- **Contract** — `@return? mtk::CLOSED, mtk::EMPTY` — `mailbox.c3:246`.
- **Checks** — `mtk::@check(slot.is_empty(), "Part 9.2 rule 3: an acquisition asserts the Slot is empty on entry")` — `mailbox.c3:250`.
...
- **Contract** — `@param slot : "an empty Slot; filled on success"` — `mailbox.c3:285`.
- **Contract** — `@param timeout : "how long to wait"` — `mailbox.c3:286`.
- **Contract** — `@return? mtk::CLOSED, mtk::TIMEOUT, mtk::WOKEN` — `mailbox.c3:287`.
- **Checks** — `mtk::@check(slot.is_empty(), "Part 9.2 rule 3: an acquisition asserts the Slot is empty on entry")` — `mailbox.c3:291`.
...
- **Contract** — `@param out : "an empty queue; every queued item is moved onto it, in receive order"` — `mailbox.c3:352`.
- **Contract** — `@return? mtk::CLOSED` — `mailbox.c3:353`.
...
- **Contract** — `@return? mtk::CLOSED` — `mailbox.c3:375`.
...
- **Contract** — `@param out : "an empty queue; the remainder is moved onto it, in receive order"` — `mailbox.c3:407`.
...
- **Contract** — `@param want : "the identity asked for"` — `pool.c3:69`.
- **Contract** — `@param in_pool : "how many of this identity remain, AFTER the removal. A hint, and stale. Part 12.4"` — `pool.c3:70`.
- **Contract** — `@param slot : "empty on entry; fill it or leave it"` — `pool.c3:71`.
...
- **Costs** — called with no lock held. Items added to `extra` are taken the
  ...
  back.
- **Contract** — `@param in_pool : "how many of this identity are held, BEFORE the addition. A hint. Part 12.4"` — `pool.c3:85`.
- **Contract** — `@param slot : "full on entry"` — `pool.c3:86`.
- **Contract** — `@param extra : "an empty queue; items added here are taken the same way, with the same checks. Part 12.5, the composite mechanism"` — `pool.c3:87`.
...
- **Costs** — **called once by `close`, and possibly once more with
  ...
  on the first call.**
- **Contract** — `@param remaining : "everything the pool still held, flattened across every identity. No order is promised — R12"` — `pool.c3:107`.
...
- **Contract** — `@param a : "the allocator the pool keeps for life"` — `pool.c3:214`.
- **Contract** — `@param tags : "the identities this pool holds. Not empty, and no duplicates — both checked"` — `pool.c3:215`.
- **Contract** — `@param hooks : "the policy. Part 12"` — `pool.c3:216`.
- **Checks** — `mtk::@check(tags.len > 0, "Part 11.7: the set of identities is not empty")` — `pool.c3:220`.
- **Checks** — `mtk::@check(hooks != null, "Part 12.1: a pool cannot exist without hooks")` — `pool.c3:221`.
- **Checks** — `mtk::@check(t != u, "Part 11.7: the pool's set of identities has a duplicate")` — `pool.c3:240`.
...
- **Costs** — two frees.
- **Checks** — `always_assert(self._closed, "Part 11.12: releasing an open pool")` — `pool.c3:275`.
...
- **Contract** — `@param want : "the identity wanted"` — `pool.c3:318`.
- **Contract** — `@param mode : "which of Part 11.7's three modes"` — `pool.c3:319`.
- **Contract** — `@param slot : "an empty Slot; filled on success"` — `pool.c3:320`.
- **Contract** — `@return? mtk::CLOSED, mtk::NOT_AVAILABLE, mtk::NOT_CREATED, mtk::UNKNOWN_IDENTITY` — `pool.c3:321`.
- **Checks** — `mtk::@check(slot.is_empty(), "Part 9.2 rule 3: an acquisition asserts the Slot is empty on entry")` — `pool.c3:325`.
- **Checks** — `mtk::@check(b != null, "Pool.get for an identity the pool was not created with")` — `pool.c3:334`.
- **Checks** — `mtk::@check(slot.peek().link.type == want, "Part 12.2: the get hook returned an item of a different identity")` — `pool.c3:363`.
...
- **Contract** — `@param want : "the identity wanted"` — `pool.c3:391`.
- **Contract** — `@param slot : "an empty Slot; filled on success"` — `pool.c3:392`.
- **Contract** — `@param timeout : "how long to wait"` — `pool.c3:393`.
- **Contract** — `@return? mtk::CLOSED, mtk::TIMEOUT, mtk::UNKNOWN_IDENTITY` — `pool.c3:394`.
- **Checks** — `mtk::@check(slot.is_empty(), "Part 9.2 rule 3: an acquisition asserts the Slot is empty on entry")` — `pool.c3:398`.
- **Checks** — `mtk::@check(b != null, "Pool.get_wait for an identity the pool was not created with")` — `pool.c3:416`.
...
- **Contract** — `@param slot : "a full Slot; cleared if the pool took the item"` — `pool.c3:460`.
- **Checks** — `mtk::@check(b != null, "Pool.put of an identity the pool was not created with")` — `pool.c3:474`.
- **Checks** — `mtk::@check(b != null, "the put hook returned an identity the pool was not created with")` — `pool.c3:561`, reached through the private store path for both the Slot and each item of `extra`.
