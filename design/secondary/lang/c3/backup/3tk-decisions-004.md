...
- **`take()` hands over everything the queue holds and empties the source, O(1),
  and cannot fail.** Written for `Pool.on_close`'s by-value hand-off, `P6`,
  built by 3TK-56. `../3tk/src/queue.c3:161`.
...
- **`on_close` takes the queue by value, not by pointer.** *I do not care what
  you did* — the pool hands the items over and keeps nothing: no pointer, no
  count, no check. `P6` (`3tk-open-defects.md`), ruled 2026-08-28, built by
