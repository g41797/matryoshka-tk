...
`3tk-status.md` records that 3TK-5 waits on the owner for two things: the
ruling on C1 to C11, and the Q4 decision. Neither had been given when this
stage ran.

A porting proposal is the document where such rulings are *proposed*. So every
open decision is decided here, argued, and marked:

> **PROPOSED — owner overridable.**

Nothing in this file is a settled ruling until the owner says so. Where an
override changes other decisions, the dependency is named on the spot.

Section 2 is the decision register: sixteen decisions, each with the
alternative it rejects and the cost of the override.
...
```
mtk/
  any.c3          AnyNode, AnyHandle, Slot, the inner and the identity
  helper.c3       mtk::helper <Type>   — generic module, the per-type border
  owned.c3        mtk::owned  <Type>   — helper plus create and release
  list.c3         NodeList — the intrusive list and its checks
  mailbox.c3      Mailbox
  pool.c3         Pool, PoolHooks
  mtk.c3          the one import, re-exporting the above
```

Seven files. Four of them are the toolkit of Part 17.1; `mailbox.c3` and
`pool.c3` are the two optional tools of Part 17.2; `mtk.c3` is the front door.
...
**Ruling, in three parts. PROPOSED.**
...
**Ruling: no `MboxHandle`, no `PoolHandle`. One `AnyHandle`. PROPOSED.**
...
**Ruling: `typedef Slot = AnyHandle`. Distinct, not an alias. PROPOSED.**
...
```c3
macro @check(bool #cond, String msg) @private
{
...
**Ruling: `wait_timeout` does not appear in the port. PROPOSED, and it is
closer to a finding than a decision.**
...
**Ruling: Part 2.9 is not implemented. PROPOSED.**
...
**Ruling: `mtk::helper <Type>` and `mtk::owned <Type>`. PROPOSED.**
...
**Ruling: Part 22 as written. The list is step 5. PROPOSED.**
...
| Spec | Marking | C3 shape |
|---|---|---|
| 11.7 The pool | MUST | One `NodeList` per identity plus one count, in a `HashMap{typeid, Bucket}` fixed at creation and asserted non-empty |
...
|---|---|---|
| Safe, unoptimized | default | Tiers 1, 2 and 3 all live. Every check runs |
| Safe, optimized | `-O3` | The same, under an optimizer |
...
Eleven closed, four of them by 3TK-4 and seven here. Every one of the seven is
PROPOSED and overridable.

---

# 9. What 3TK-6 would be

Not authorized. Named so the owner can name it.

The proposal's own porting order is Part 22, and steps 2 to 5 are one coherent
piece of work: `any.c3`, `helper.c3`, `owned.c3`, `list.c3`, with the tests of
7.3 that apply to them, in the four builds of 7.2. That is the toolkit of Part
17.1 — the thing without which there is nothing to port — and it is the first
stage that writes C3.

Steps 6 and 7, the two containers, are a second piece and depend on nothing in
the first beyond its public surface, by Part 17.2.

The owner decides whether the rulings of section 2 are accepted, amended, or
overridden, before either.
