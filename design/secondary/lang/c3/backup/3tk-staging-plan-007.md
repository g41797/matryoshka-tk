...
**Version 007.** It supersedes `3tk-staging-plan-006.md`, and that one 005,
004, 003, 002 and 001. All stay on disk, in `backup/`. The only change in 007 is
the addition of **3TK-12 and 3TK-13**, on the owner's instruction of 2026-08-24.
**Both are declared and neither is authorized.** They are written as one
sequence, audit then cut, so the order cannot drift. Stages 3TK-0 to 3TK-11 are
reproduced unaltered — they are done, and a plan version does not rewrite
history.

**Why these two, and why now.** 3TK-11 rewrote the core in the shape R1 to R15
ruled, and `../common/` was deliberately left untouched. So **the port is ahead
of the specification every port reads.** R14 already ruled that the
specification moves to 003. What 007 adds is the step before it: the difference
between the code and specification 002 has to be *measured*, per Part, against
`3tk/src/` — because proposal 002 §8.1 is a **prediction** of what would change,
and 3TK-11 found three places where 002 was wrong in detail. A specification cut
from a prediction inherits the prediction's mistakes into dtk and otk.
