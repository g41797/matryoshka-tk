# What the C3 port decided, and why (001)

Stage 3TK-20, 2026-08-24. **Description and reasoning. Not recommendations.**
...
`Inner` is two fields — `inner.c3:58-62`:
...
own words and no code depends on it.
...
null off one**, and the membership question becomes one load — `inner.c3:190`:

```c3
fn bool is_linked(Handle h) @inline => h != null && h.link != null;
```

Every walk ends at `n.link == n` rather than at null: `queue.c3:122`,
`queue.c3:182`, `queue.c3:216`, `stack.c3:124`.
...
[3tk-any-options-001.md](3tk-any-options-001.md). `next` asserts *the following
item*, which the field is not when it points at itself.
...
Tier 2 is four lines — `inner.c3:145-151`:
...
tier 3 flag is the pool's duplicate-identity scan at creation —
`inner.c3:152-159`.
...
type named there — `helper.c3:76-226`. A new outer type costs nothing before it
can be used.
...
|---|---|---|
| 001 | 2026-08-24 | First version. Stage 3TK-20. Written from R1 to R15, D1 to D16, V1 to V19, P1 to P6, `3tk/src/`, `src/*.zig` at the repository root, and 004's change log. Describes; recommends nothing. |
