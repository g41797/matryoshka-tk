...
- **Three tiers, one port macro.** `D6`. Tier 2 is `mtk::@check`
  ...
  `../3tk/src/mailbox.c3:96` and `../3tk/src/pool.c3:215`.
...
- **A container keeps its allocator for life, and no release call takes one.**
  `D3`, `Part 13.1`. `../3tk/src/mailbox.c3:92`, `../3tk/src/pool.c3:211`.
...
- **A participant is allocated once and lives for the duration of the run.**
  `Part 3.1`. `../3tk/src/mailbox.c3:68`, `../3tk/src/pool.c3:163`.
...
- **One wait-loop shape, used by every waiting call.** The deadline is anchored
  ...
  leaver that times out passes the signal on. `D7`. `../3tk/src/mailbox.c3:218`,
  `../3tk/src/pool.c3:314`.
- **`wait_timeout` is banned in the port.** It recomputes the deadline on every
  call. `D7`, `Part 2.5`. `../3tk/src/mailbox.c3:224`.
- **The pre-lock fast read is kept, and the re-read under the lock is not
  optional.** `D16`, `Part 15.4`. `../3tk/src/mailbox.c3:108`,
  `../3tk/src/pool.c3:228`.
...
- **The one hard gate is at build time:** `required_alloc_offset` refuses a
  ...
  `Part 7.4`. `../3tk/src/managed.c3:66`.
...
- **The pool keeps one per identity, and it is the only `InnerStack` in the
  port.** It never crosses the public surface. `R2`, `R11`, `R13`.
  `../3tk/src/stack.c3:4`.
...
- **There is no Slot-shaped insert.** `push_slot` had no caller it could ever
  acquire and was deleted 2026-08-24, on the owner's instruction. `R13`, `P6`.
  `../3tk/src/stack.c3:59`.
...
- **`extra` is the composite mechanism**: items added there are taken the same
  way, with the same checks. `Part 12.5`. `../3tk/src/pool.c3:68`.
- **`on_close` is called once by `close`, and possibly once more with
  ...
  weakens. `V11`. `../3tk/src/pool.c3:82`.
...
- **The count handed to a hook is a hint and is stale** — after the removal on
  ...
  `../3tk/src/pool.c3:52`, `../3tk/src/pool.c3:68`.

### Creation and release

- **The hooks are a parameter of creation, not a later step.** A pool cannot
  exist without them. `Part 12.1` MUST. `../3tk/src/pool.c3:163`.
...
- **Everything read before the pool unlocks for a hook is stale when the hook
  returns.** `Part 12.3`. `../3tk/src/pool.c3:291`.
- **No lock is held across a call into application code.** `Part 12.3`,
  `Part 15.2`. `../3tk/src/pool.c3:291`.
- **`get_wait` never creates. No hook is called on that path**, and where a
  ...
  `Part 11.9` calls the divergence deliberate. `../3tk/src/pool.c3:314`.
...
- **`get_wait` reports `UNKNOWN_IDENTITY` too, on two grounds that are not
  ...
  problem. `A3`. `../3tk/src/pool.c3:328`.
- **`get_wait` is the one place that holds a `PoolBucket*` across a release of
  ...
  re-read every turn. `../3tk/src/pool.c3:328`.
...
- **`put` re-reads the closed flag after the hook.** `Part 12.3` MUST forces
  ...
  window. `P1`, ruled 2026-08-24. `../3tk/src/pool.c3:397`.
- **One rule for that window, and it is the pool's own: what the pool holds
  ...
  comes back to the caller. `P1`, `V11`. `../3tk/src/pool.c3:397`.
- **The alternative, restoring the caller's Slot, cannot carry `extra`**, whose
  items the caller never had. `../3tk/src/pool.c3:397`.
...
- **That flatten is `pop` then `push_back`, O(n) once, on a pool going down**,
  ...
  have had to walk and do anyway. `R12`. `../3tk/src/pool.c3:456`.
- **No order is promised, and `push_back` is chosen for being the simplest
  write.** `R12`. `../3tk/src/pool.c3:456`.
...
- **`required_alloc_offset` is declared twice, identically.**
  `../3tk/src/inner.c3:189` and `../3tk/src/managed.c3:66` carry the same macro
  and the same doc comment. Both call sites — `../3tk/src/managed.c3:32` and
  `../3tk/src/managed.c3:52` — name the `mtk::` one. Whether the second
  declaration is reachable at all, and which of the two should stay, is not a
  decision any document has taken, so it is not taken here.
