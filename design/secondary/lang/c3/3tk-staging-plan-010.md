...
**Version 010.** It supersedes `3tk-staging-plan-009.md`, and that one 008, 007,
006, 005, 004, 003, 002 and 001. All stay on disk, in `backup/`. The only change
in 010 is the addition of **3TK-18 and 3TK-19**. Stages 3TK-0 to 3TK-17 are
reproduced unaltered — every one of them has run, and a plan version does not
rewrite history. `3tk-status.md` is the live record of what is done.

**Why these two, and why now. Plan 009 is spent** — 3TK-0 to 3TK-17 have all
run, `3tk/` is green at 63 checks and four builds, and nothing in this line is
declared and unstarted. Two things are known to be owed and neither has a stage
to run under.

- **A decision exists and nothing carries it into the code.** The owner ruled
  `any` within `Inner` on 2026-08-24: **rejected in all three shapes**, with one
  rename accepted — `Inner.next` becomes `Inner.link`. That is **3TK-18**, and
  it is the same shape as 3TK-11 and 3TK-16 — a ruled document, built.
- **Three small debts survive 3TK-17 and each was left for a stage the owner
  names.** Stale 003 citations in `3tk/src/`, a stale P2 row in the deviations
  audit, and otk never having been told the specification exists. That is
  **3TK-19**, and it is bookkeeping, not design.

**The order is the numbering, and there is only one constraint.**

```
3TK-18  (the rename, code)   →   3TK-19  (the three debts)
```

**3TK-19 runs after 3TK-18**, for the reason 3TK-15 ran after 3TK-16: 3TK-18
rewrites the prose of `inner.c3`, and one of 3TK-19's three stale citations
lives in that file. 3TK-18 is permitted to take that one on the way past — the
paragraph is being rewritten anyway — and 3TK-19 then covers what is left and
says which.
...
- this plan — `3tk-staging-plan-NNN.md`, currently **010**. *(008 and 007 both
  carried this line reading `currently 007`. It was stale in both; corrected in
  009 and kept current here — the line is worth one edit per version.)*
- `ztk-audit-NNN.md`
...
- **3TK-18 and 3TK-19 write no file under `../common/`**, and 3TK-18 writes no
  new document at all — the ruling it builds is already written. Both still end
  with `3tk/run-builds.sh` green and the check count stated.
