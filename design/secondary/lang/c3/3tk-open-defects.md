...
- `matryoshka-3tk/design/3tk-decisions-006.md` — the `P6` entry, under `pool.c3` and `queue.c3`.
  `-003` went to `backup/`.
...
**`matryoshka-3tk/design/3tk-decisions-006.md` is the current one**, and every earlier version is in `backup/`.
