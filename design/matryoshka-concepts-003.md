# Matryoshka Concepts (003)


Change from -002: INTR 8 — Chapter 4 names the two classes of item. The toolkit  
has had both since Layer 2, and no document said so.
...
- [rules-049.md](rules-049.md) — coding, doc, and process rules.
- [patterns-029.md](patterns-029.md) — the pattern and idiom catalog.
...
Everything that travels through Matryoshka is a PolyNode. Not everything that  
is a PolyNode travels.
...
- `PolyHelper` generates no `create` and no `destroy` for them. Both types
  declare `no_create_destroy`, because both allocate themselves.
