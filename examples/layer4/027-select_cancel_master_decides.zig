...
const CancelDecideMaster = struct {
    ...
    fn phase2Receive(self: *CancelDecideMaster) !usize {
        ...
        try sel2.concurrent(.inbox2, mailbox.receiveResult, .{ self.mbx2, null });
    ...
    fn destroy(self: *CancelDecideMaster) void {
        ...
        mailbox.destroy(self.mbx1, self.allocator);
        var rem2: polynode.ItemList = self.mbx2.close();
        items.freeList(&rem2, self.allocator);
        mailbox.destroy(self.mbx2, self.allocator);
